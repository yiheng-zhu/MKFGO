...
class MLP(object):
    ...
    def __init__(self, workdir, go_type, round_index):
        ...
        self.model_dir = os.path.join(glm_data_dir, go_type, glm_model_name, str(round_index))
        self.result_dir = os.path.join(workdir, go_type, glm_result_dir, glm_round_name + str(round_index))
        self.term_list_file = os.path.join(glm_data_dir, go_type, glm_term_list)
    ...
    def dnn(self, x, keep_prob, is_train):   # deep neural network
        ...
        x = tf.layers.dense(inputs=x, units=glm_full_connect_number, activation=tf.nn.relu)
        x = tf.nn.dropout(x, keep_prob)
        x = tf.layers.batch_normalization(x, training=is_train)
        y_pred = tf.layers.dense(inputs=x, units=go_label_size_small[self.go_type], activation=tf.nn.sigmoid)
    ...
    def process(self, x, y, keep_prob, is_train):   # process
        ...
        with tf.name_scope('adam_optimizer'):
            ...
            with tf.control_dependencies(update_ops):
                train_step = tf.train.AdamOptimizer(glm_learning_rate).minimize(cross_entropy)
    ...
    def running(self): # main process
        ...
        x = tf.placeholder(tf.float32, [None, glm_feature_size])
        y = tf.placeholder(tf.float32, [None, go_label_size_small[self.go_type]])
        keep_prob = tf.placeholder(tf.float32)
        is_train = tf.placeholder(tf.bool)

        train_step, cross_entropy, y_pred = self.process(x, y, keep_prob, is_train)

        test_data_list = rd.create_batch(self.test_feature, self.test_label, self.test_name_list, glm_batch_size, False)
        ...
        with tf.Session(config=config) as sess:
            ...
            test_pred_matrix = np.zeros([1, go_label_size_small[self.go_type]])
            ...
            sh.save_cross_entropy_results(self.result_dir, self.go_type, self.term_list_file, glm_result_name, self.test_name_list, test_pred_matrix)
