...
parser.add_argument('--name_list', default='', type=str, help='file list path ')
parser.add_argument('--seq_path', default=' ', type=str,help=' define the path for sequences')
parser.add_argument('--esm1b_path', default=' ', type=str,help=' define the path for esm1b features')
parser.add_argument('--prottrans_path', default=' ', type=str,help=' define the path for prottrans features')
parser.add_argument('--model_path', default=' ', type=str,help=' define the path for prediction models')
parser.add_argument('--save_path', default=' ', type=str,help=' define the save path for prediction results')
parser.add_argument('--device', default='cpu', type=str,help=' define the device you want the ')

args = parser.parse_args()

test_set = Proteins_Dataset(args.name_list, args.seq_path, args.esm1b_path, args.prottrans_path)  ## spot-1d test set
...
model1.load_state_dict(torch.load(os.path.join(args.model_path, "model1.pt"), map_location=torch.device('cpu')))
model2.load_state_dict(torch.load(os.path.join(args.model_path, "model2.pt"), map_location=torch.device('cpu')))
model3.load_state_dict(torch.load(os.path.join(args.model_path, "model3.pt"), map_location=torch.device('cpu')))
model4.load_state_dict(torch.load(os.path.join(args.model_path, "model4.pt"), map_location=torch.device('cpu')))
model5.load_state_dict(torch.load(os.path.join(args.model_path, "model5.pt"), map_location=torch.device('cpu')))
model6.load_state_dict(torch.load(os.path.join(args.model_path, "model6.pt"), map_location=torch.device('cpu')))
